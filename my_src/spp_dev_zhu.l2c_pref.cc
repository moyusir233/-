...
void CACHE::l2c_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type)
{
    ...
    do {
        ...
        PT.read_pattern(curr_sig, delta_q, confidence_q, perc_sum_q, lookahead_way, lookahead_conf, pf_q_tail, depth, addr, base_addr, train_addr, curr_ip, train_delta, last_sig);
        ...
        for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
            ...
            if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same physical page
                ...
                if (FILTER.check(pf_addr, train_addr, curr_ip, fill_level, train_delta + delta_q[i], last_sig, curr_sig, confidence_q[i], perc_sum, (depth-1))) {
					...
					//[DO NOT TOUCH]:	
					prefetch_line(ip, addr, pf_addr, ((fill_level == SPP_L2C_PREFETCH) ? FILL_L2 : FILL_LLC)); // Use addr (not base_addr) to obey the same physical page boundary
...
void PATTERN_TABLE::read_pattern(uint32_t curr_sig, int *delta_q, uint32_t *confidence_q, int32_t *perc_sum_q, uint32_t &lookahead_way, uint32_t &lookahead_conf, uint32_t &pf_q_tail, uint32_t &depth, uint64_t addr, uint64_t base_addr, uint64_t train_addr, uint64_t curr_ip, int32_t train_delta, uint32_t last_sig)
{
    // Update (sig, delta) correlation
    uint32_t set = get_hash(curr_sig) % PT_SET,
             local_conf = 0,
    ...
    if (c_sig[set]) {
        for (uint32_t way = 0; way < PT_WAY; way++) {
            ...
            if (pf_conf && pf_q_tail < L2C_MSHR_SIZE && do_pf ) {
...
void get_perc_index(uint64_t base_addr, uint64_t ip, uint64_t ip_1, uint64_t ip_2, uint64_t ip_3, int32_t cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth, uint64_t perc_set[PERC_FEATURES])
{
	...
	for (int i = 0; i < PERC_FEATURES; i++) {
		perc_set[i] = (pre_hash[i]) % PERC.PERC_DEPTH[i]; // Variable depths ，将特征值变成深度的映射，这里的深度就是条目数量，perc_set就是映射到条目中
		SPP_DP (
...
int32_t	PERCEPTRON::perc_predict(uint64_t base_addr, uint64_t ip, uint64_t ip_1, uint64_t ip_2, uint64_t ip_3, int32_t cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth)
{
    ...
    // 设置各个特征的权重
    // Featrure Weight
    int32_t FEAT_IMP[PERC_FEATURES];
    FEAT_IMP[0] = 6;
    FEAT_IMP[1] = 9;
    FEAT_IMP[2] = 8;
    FEAT_IMP[3] = 9;
	...
	int32_t sum = 0;
	for (int i = 0; i < PERC_FEATURES; i++) {
		sum += perc_weights[perc_set[i]][i] * FEAT_IMP[i];
		// Calculate Sum
	}
	SPP_DP (
